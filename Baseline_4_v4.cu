...
__global__ void radixSort1bit(uint32_t * in, int n, uint32_t * out,int indexbit, int bit,int nBins)
{   
    extern __shared__ uint32_t value[];
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) 
    ...
    int nBits, int  blockSize)
{
    ...
    for (int bit = 0;  bit < sizeof(uint32_t) * 8; bit += nBits)
    {
        ...
        {
            radixSort1bit<<<gridSizeHist,blockSizeHist,blockSize*sizeof(uint32_t)>>>(d_in,n,d_out,indexbit,bit,nBins);
            uint32_t* d_temp=d_in;
            d_in=d_out;
            d_out=d_temp;        
        }
